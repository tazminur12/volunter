...
const UpdatePost = () => {
  ...
  const { usePost, useUpdatePost } = usePostQueries();
  const updatePostMutation = useUpdatePost();
  const [error, setError] = useState(null);

  // Use React Query to fetch post
  ...
  useEffect(() => {
    if (post && user && post.organizerEmail !== user.email) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updatePostMutation.mutateAsync({ id, postData: updatedPost });
      navigate('/dashboard/my-posts');
    } catch (err) {
