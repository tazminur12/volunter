...
const PostDetails = () => {
    ...
    const { usePost, useApplyForPost, useDecreaseVolunteerCount } = usePostQueries();
    const { data: post, isLoading: loading, error } = usePost(id);
    const applyForPostMutation = useApplyForPost();
    ...
    const handleApply = async () => {
        ...
        try {
            ...
            await applyForPostMutation.mutateAsync(requestData);
            
            // Try to decrease volunteer count
            try {
                await decreaseVolunteerCountMutation.mutateAsync(post._id);
            } catch (decreaseError) {
                console.warn('Failed to decrease volunteer count:', decreaseError);
                // Still show success as the application was submitted
            }
